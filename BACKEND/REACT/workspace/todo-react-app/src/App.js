...
function App() {
  ...
  //   )
    ...
    // => 최초 App.js 가 실행되어 fetch()로 비동기 호출함
    // => 응답이 오면 .then()이 실행됨
    // => 이때, setItems()로 값이 변경되면 다시 랜더링 하기 위해 App.js 가 실행됨
    // => App.js가 실행되면 fetch()를 또 실행함 (무한루프...)
    // 이러한 현상을 방지해 주는것이 effect 훅
    // 리액트 훅 중에서 Effect 훅인 useEffect() 함수를 사용하여 무한루프 해결! 
    // useEffect(콜백함수, 배열)
    // => 첫랜더링이 일어났을때, 그 이후에는 배열안(두번째 파라미터)의 값이 변할때마다 
    //    콜백함수(첫번째파라미터)를 호출한다.
    ...
    //       (error) => {}    // error 함수 
    //     )
    // }, []) // 두번째 파라미터 배열을 주시하고있음 , 배열이 변하면 다시 콜백함수 호출
