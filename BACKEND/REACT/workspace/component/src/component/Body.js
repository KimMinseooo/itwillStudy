...
function Body () {
    ...
    // return (
        ...
        //     <h3>{n1 +n2}</h3>
        //     <h3>{str + str2}</h3>
        //     {/* boolean 타입은 표현식으로 화면에 표시할 수 없음으*/}
        ...
        //     {/* <h3>{obj}</h3>*/}
        //     <h3>a + b = {obj.a  + obj.b}</h3>
        // </div>

        // < JSX 규칙 >
        // 1. 반드시 닫는 태그가 있어야함
        // ex) <input> (x), <input></input> (O) , <input /> (O)
        // 2. 반드시 하나으 ㅣ요소를 리턴! (하나도 없어도 에러!)
        // 단, <React.Fragment> 태그로 감싸면 가능 (React import 필요함!)
        // <React.Fragment>
        //     <div>1</div>
        //     <div>2</div>
        // </React.Fragment>
    ...
    return (
        ...
        // 주의! JSX는 HTML과 JS가 섞이다 보니 정확히 구분할 필요가 있음
        // 따라서, background-color 대신 backgroundColor를 써야함! 
        // <div style="background-color: red; color: blue;"></div>
        // => 분명 HTML에서 style이 맞지만 지금 확장자가 JSX이므로 style 속성을
        // => 따라서 , {{}} 가 싫고 css문법을 사용하고 싶다면 css파일을 따로 분리
