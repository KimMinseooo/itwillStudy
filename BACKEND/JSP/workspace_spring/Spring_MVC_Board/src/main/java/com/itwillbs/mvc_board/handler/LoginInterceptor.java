...
public class LoginInterceptor implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// HttpSession 객체에서 로그인 세션 아이디 (sId) 꺼내서 null 값 인지 판별
		String id = (String)session.getAttribute("sId");
		
		if (id == null) {
			
			// result/fail.jps 페이지로 전달할 파라미터 값을 Model 객체에 저장했었지만
...
//			return "result/fail";
			...
			// 컨트롤러의 메서드처럼 ViewResolver 를 통해 뷰페이지 포워딩 처리 설정이 불가능하다!
			// ----------------------------------------------------------
			// 인터셉터 내의 preHandleo() 메서드 리턴타입이 boolean인 이유는
			...
			// => 주의! 이동할 뷰페이지 지정 시 ViewResolver의 prefix, suffix 값을 활용하지 못하도로
	...
	// => 예외(Exception) 등으로 인해 return 문이 실행되지 않을 경우 호출 X
	@Override
	...
	// 요청이 모두 완료된 후 예외와 관계없이 클라이언트에 응답이 전달될때 실행됨
	@Override
