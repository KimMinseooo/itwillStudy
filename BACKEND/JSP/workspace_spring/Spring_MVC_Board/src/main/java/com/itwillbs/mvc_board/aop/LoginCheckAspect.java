...
public class LoginCheckAspect {
	...
	// => 메서드 정의 시 리턴타입은 상황에 맞게 지정 (void도 가능) 
	// => 단, 메서드 파라미터로 스프링 빈으로 관리되는 객체 지정 불가능 (컴파일 에러는 없지만 실행 X)
...
//	public void loginCheck() {}
	...
	// 메서드 파라미터로 어노테이션 타입 변수를 설정하되, 변수명을 @annotation(변수명)rhk ehddlfgkrp wlwjd
	...
	public void loginCheck(LoginCheck loginCheck) {
		...
		System.out.println(">>>>>>>>>> 회원 타입 : " + loginCheck.memberRole());
...
//		}
		...
		if(id == null) {
			...
			// 해당 예외 처리시 포워딩이나 리다이렉트를 수행하도록 할 수도 있다!
			// => 이때 , HttpStatusCodeException 클래스 활용 시 특정 HTTP 상태 코드를 예외로 발생 가능
			//	 (인가받지 못한 사용자일 때 UNAUTHORIZED 상수가 딱 맞아 떨어짐(THTP 401))
