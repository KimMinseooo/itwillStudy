 ...
 * 1) fixedRate : 밀리초 단위로 설정한 시간 간격으로 스케줄링 실행됨
...
//	@Scheduled(cron = "0 0 10 5 * *") // 매월 5일 오전 10시 마다 작업이 실행됨
//	@Scheduled(cron = "0 0 10 5W * *") // 매월 5 일에 가장 가까운 평일 오전 10시마다 작업이 실행됨
	@Scheduled(cron = "0 0 9-18/1 * * *") // 매일 9 ~ 18시까지 1시간 간격으로 작업이 실행됨
	public void test() {
		System.out.println("===== 스케줄러 작업 =======");
		System.out.println("===== 스케줄러 작업 =======");
		System.out.println("===== 스케줄러 작업 =======");
