...
public class TestServlet5 extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 브라우저 주소 표시줄의 요청 주소가 "http://localhost:8080/StudyJSP/show.test5" 
		...
		if (command.equals("/main.test5")) {
			...
			//    jsp 페이지의 내용만 서버에서 처리 후 응답 데이터로 전송하려면
			//    디스패치(Dispatch) 방식의 포워딩 수행!
			// 1) request 객체의 getRequestDispatcher() 메서드를 호출하여 
			//    RequestDispatcher 객체를 리턴받기(파라미터 : 포워딩할 주소(파일명))
			request.setAttribute("data","서버로부터 전송받은 데이터" );
			RequestDispatcher dispatcher=request.getRequestDispatcher("jsp09_servlet/test5_main.jsp");
			...
			// JSP 페이지를 통해 응답 데이터를 사용자엑세 전송하여 출력해야한다!
			
			request.setAttribute("data","서버로부터 전송받은 데이터" );
			RequestDispatcher dispatcher=request.getRequestDispatcher("jsp09_servlet/test5_list.jsp");
			dispatcher.forward(request, response);
		}
	}
}
