...
<body>
	...
	1) action 속성
		- submit 동작 시 폼 태그 내의 입력값(= 폼 파라미터)를 모두 HTTP 요청 메세지에 저장 후 
		  ...
		  => 이떄 , HTTP 요청 메세지 형태로 전송
	2) method 속성
		 - GET 방식 (method="get" => 기본값)
		 	=> URL에 파라미터가 함꼐 포함되어 전송되는 요청 방식
	...
	<form action="request1_pro.jsp" method="post">
		...
		<%-- 전송(submit) 버튼 클릭 시 form 태그 내의 action 속성에 기술된 페이지로 이동
			이때, GET 방식 요청(method="get")에 의해 입력된 폼 파라미터 데이터가
			모두 요청 URL(주소) 뒷 부분에 연결되어 함께 전송됨
			 --%>
	</form>
