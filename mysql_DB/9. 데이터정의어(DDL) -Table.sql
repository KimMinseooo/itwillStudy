...
-- (2) 제약조건
-- 테이블의 특정 컬럼에 부적합한 데이터가 삽입/수정/삭제되는 것을 막아줌.
...
-- 고유값(주민번호, 전화번호, 이메일 등)이 들어와야하는 컬럼에 선언함.
-- test2 테이블 생성하기 
create table test2
...
-- pk 제약족너이 선언된 컬럼에는 null값도 안되고, 중복값도 안됨.
-- 단, 테이블당 한번만 선언 가능함!
-- <stu> 테이블
...
-- create talbe + 서브쿼리 예제
-- 기존 테이블ㄹ을 기반으로 복사본의 테이블이 생성됨.(단 , 제약조건은 not null 제약조건만 복사됨)
-- 주로 테이블 백업 또는 테스트용 테이블 생성할때 활용한다.
create table dept80
...
-- default값이 있는 컬럼은 다동으로 default값 저장됨.
alter table dept80
add email varchar(25) default '미입력';

-- 테이블 가장 앞에 또는 특정 컬럼 뒤에 컬럼 추가하는 방법
alter table dept80
...
alter table dept80
add age int not null;            		-- 자동으로 null 값 대신 0 삽입

alter table dept80
add join_date date not null;		     -- 자동으로 null 값 대신 0000-00-00 삽입
...
insert into dept80 
values (null, 180, "Hong",3000,80000,'2024-12-18',null,default,'010-1234-5678',25);
...
alter table dept80
drop primary key;
-- [문법2] alter table 테이블명 drop --; : fk , ck, uk
use information_schema;
select *
from table_constraints
...
-- [문법3] alter table 테이블명 drop --; : not null
