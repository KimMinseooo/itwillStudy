...
-- (==)
select e1.employee_id as emp_id , e1.last_name as emp_name ,
	   e2.manager_id as mgr_id , e2.last_name as mgr_name
from employees e1 join employees e2
on e1.manager_id = e2.employee_id
order by e1.employee_id;
...
-- <연습문제>
-- 1.employees 테이블과 departments 테이블을 조인하여 모든 사원의 정보와 함께 부서 정보를 함께 출
-- 력하시오
select e.employee_id,e.last_name, e.salary,
...
-- 2. employees 테이블로부터 모든 사원의 last_name, employee_id, 매니저 이름, manager_id를 함께 출
-- 력하시오
...
-- 각 부서의 매니저 이름(last _name)을 함께 출력하시오
select e1.employee_id, e1.last_name, e1.salary ,
	   d.department_id, d.department_name, d.manager_id,
       e2.last_name as "dept_egr_NAME"
...
-- 주문자의 정보(member_name, phone, address) 를 함꼐 출력하시오
...
-- 출력형식 : order_num,order_date,member_name,phone, address,
...
select m.member_id, m.member_name, m.phone , m.address, 
		o.order_num, o.order_date, o.prod_id, p.prod_name 
from orders o right outer join members m
on  o.member_id= m.member_id
left outer join products p 
on o.prod_id = p.prod_id
order by o.member_id;
